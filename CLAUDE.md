...
- **Pi** (`pi/`): MediaMTX systemd service `frogwatch-cam` reads the camera (`rtsp://localhost:8554/cam`, 1280x720@25 hardware H.264) and pushes via `runOnReady: ffmpeg -c copy` to the VPS — SRT primary (`PUSH_MODE=srt`), RTSP/TCP fallback. Provisioned by `pi/setup.sh` (idempotent, run via `ssh raspi 'sudo bash -s' < pi/setup.sh`); it renders `pi/mediamtx.yml.tpl` with envsubst from `/etc/frogwatch/frogwatch.env` into `/etc/frogwatch/mediamtx.yml`. setup.sh fetches the template/unit from GitHub master when run via stdin — push changes before re-provisioning.
...
- **Frontend** (`front/`): Svelte 5 + Vite, no SSR. `Player.svelte` (hls.js, dynamically imported; Safari uses native HLS; self-healing retry/backoff + offline overlay), `Gallery.svelte` (fetches `/photos/index.json` = JSON array of `YYYY-MM-DD` strings, images at `/photos/<date>.jpg`).
